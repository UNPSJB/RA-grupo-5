...
def assign_roles(payload: AssignRolesIn, db: Session = Depends(get_db)):
    SeguridadService(db).assign_roles(persona_id=payload.persona_id, roles=payload.roles)

@router.get("/me/permissions", response_model=PersonaPermissionsOut)
def me_permissions(current=Depends(get_current_persona), db: Session = Depends(get_db)):
