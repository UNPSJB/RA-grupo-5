...
export default function GenerarInformeSintetico() {
  ...
  const { crearInformeSinteticoCarrera } = useInformesSinteticos(
    numCiclo ?? 0,
    cuatrimestre ?? ""
  ); // (el ciclo no se usa en la función create, pero el hook lo pide)
  const {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => setAlert((a: AlertState) => ({ ...a, exiting: true, show: false })),
      2500
    );
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        setAlert({
          ...
          exiting: false,
          variant: "success",
          message: "Informe Sintético guardado ✔",
        });
      } catch (err: any) {
        console.error(err);
        setAlert({
          ...
          exiting: false,
          variant: "danger",
          message: err?.message || "Error al guardar el informe.",
        });
      } finally {
        setSaving(false);
      }
    ...
    [
      ...
      numCiclo,
      cuatrimestre,
      informesFiltrados, // Datos
      comisionAsesora,
      integrantes, // Estado del form
      crearInformeSinteticoCarrera,
      guardarRespuestaSintetico, // Acciones
      navigate,
    ]
  );
  ...
  if (!carrera) {
    return (
      <Container className="mt-4 alert alert-danger">
        Error: No se pudo cargar la <b>Carrera</b>...
      </Container>
    );
  }
  if (!informeBase) {
    return (
      <Container className="mt-4 alert alert-danger">
        Error: No se pudo cargar la <b>Plantilla Base</b>...
      </Container>
    );
  }
  if (!informesFiltrados) {
    return (
      <Container className="mt-4 alert alert-danger">
        Error: <b>informesFiltrados</b> es nulo.
      </Container>
    );
  }
          ...
          <Card className="mb-4 border rounded shadow-sm">
            ...
            <Card.Body className="p-4">
              ...
              <Row>
                <Col md={6}>
                  <p>
                    <strong>Ciclo Lectivo:</strong> {numCiclo}
                  </p>
                  <p>
                    <strong>Cuatrimestre:</strong> {cuatrimestre}
                  </p>
                  <p>
                    <strong>Sede:</strong> {carrera.sede}
                  </p>
                </Col>
